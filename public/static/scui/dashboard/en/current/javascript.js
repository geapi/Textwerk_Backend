...
var a=c?c.get("canEdit"):NO;d=this._getViewClass("widgetEditViewClass");if(!d){d=SCUI.MissingWidgetView.extend({message:"Widget's edit view is missing.".loc()})
...
}}},mouseDown:function(b){var c,e,d,a;this._dragData=null;if(b&&b.which===1){c=this._itemViewForEvent(b);
if(c&&!c.getPath("content.isLocked")){a=SC.clone(c.get("layout"));a.startPageX=b.pageX;
a.startPageY=b.pageY;a.view=c;a.didMove=NO;this._dragData=a}}return YES},mouseDragged:function(d){var c,b,a=this._dragData;
if(a){a.didMove=YES;c=d.pageX-a.startPageX;b=d.pageY-a.startPageY;a.view.adjust({centerX:a.centerX+c,centerY:a.centerY+b})
}return YES},mouseUp:function(b){var e,h,g,a,f,d,c;if(this._dragData&&this._dragData.didMove){e=this._dragData.view.get("content");
c=this._dragData.view.get("layout");if(e&&c){f=c.centerX||0;d=c.centerY||0;g={x:f,y:d};
this._setItemPosition(e,g);if(e.widgetDidMove){e.widgetDidMove();a=this.get("dashboardDelegate");
if(a&&a.dashboardWidgetDidMove){a.dashboardWidgetDidMove(this,e)}}}}this._dragData=null;
...
})}.observes("canDeleteContent"),_updateItemViews:function(){var n=this.get("content");
var j=this._itemViewCache||{};var r=this._itemViews||[];var k=[];var h={};var g=this.get("dashboardDelegate");
var b=this.get("canDeleteContent");var f=this.get("widgetContainerView");var q=[],p=[];
var d=this;var m,c,s,a,e,l,o;if(n&&n.isEnumerable){n.forEach(function(t,i){s=SC.guidFor(t);
e=j[s];if(!e){l={widgetViewClass:g.dashboardWidgetViewFor(d,n,i,t)||t.get(t.get("widgetViewClassKey")),widgetEditViewClass:g.dashboardWidgetEditViewFor(d,n,i,t)||t.get(t.get("widgetEditViewClassKey")),canDeleteWidget:b,content:t,owner:d,displayDelegate:d,dashboardDelegate:g,layout:d._layoutForItemView(t),layerId:"%@-%@".fmt(SC.guidFor(d),s)};
e=d.createChildView(f,l)}k.push(e);h[s]=e})}if(!k.isEqual(r)){this.beginPropertyChanges();
this.removeAllChildren();k.forEach(function(i){d.appendChild(i)});r.forEach(function(i){if(k.indexOf(i)<0){i.set("content",null)
}});this.endPropertyChanges()}this._itemViews=k;this._itemViewCache=h},_layoutForItemView:function(c){var b=null,d,a;
if(c){d=this._getItemPosition(c)||{x:20,y:20};a=this._getItemSize(c)||{width:300,height:100};
b={centerX:d.x,centerY:d.y,width:a.width,height:a.height}}return b},_itemViewForEvent:function(i){var d=this.getPath("pane.rootResponder");
if(!d||!this._itemViewCache){return null}var c=SC.guidFor(this)+"-",a=c.length,e=i.target,f=this.get("layer"),b,h,g=null;
